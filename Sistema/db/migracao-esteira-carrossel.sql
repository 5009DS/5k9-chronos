-- ═══════════════════════════════════════════════════════════════════════════
-- UMA SEGUNDA ESTEIRA: CARROSSEL
--
-- Rode no SQL Editor do projeto Supabase do 5K9 Forms, uma vez.
-- Depois de db/migracao-roteiro-desenvolvimento.sql.
--
-- ── O QUE MUDA ────────────────────────────────────────────────────────────
-- Vídeo e carrossel compartilham o começo — alguém escreve, o cliente lê, o
-- cliente aprova — e se separam exatamente aí. Depois do texto aprovado, um vai
-- para a câmera e o outro para a prancheta:
--
--   vídeo      roteiro aprovado → a gravar → gravado → em edição
--                               → gravação aguardando aprovação → publicado
--   carrossel  roteiro aprovado → a diagramar → arte pronta
--                               → arte aguardando aprovação → publicado
--
-- ── POR QUE O BANCO PRECISA SABER ─────────────────────────────────────────
-- Duas coisas aqui dentro decidem etiqueta, e nenhuma delas passa pelo código
-- da tela:
--
--   1. A lista do que o cliente pode ver. Etiqueta fora dela é cortada no
--      servidor — as três novas precisam entrar, ou o carrossel chega no
--      celular dele sem dizer em que pé está.
--
--   2. O que a APROVAÇÃO do cliente escreve. Ela empurra a peça para a etapa
--      seguinte, e essa etapa deixou de ser uma só: "a gravar" para vídeo,
--      "a diagramar" para carrossel. Quem decide é o campo `formato` da
--      própria linha, com o mesmo padrão que lib/etiquetas.js usa.
--
-- ── O QUE NÃO MUDA, E É DECISÃO ───────────────────────────────────────────
-- A trava de ajuste continua valendo só para "gravado". Refazer uma gravação
-- custa uma diária de estúdio; refazer um card custa reabrir o arquivo. Copiar
-- a trava para "arte pronta" seria copiar a regra sem copiar o motivo dela.
--
...
-- ═══════════════════════════════════════════════════════════════════════════
-- A APROVAÇÃO ESCOLHE A ETAPA PELA ESTEIRA DA PEÇA
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ── Conferência ───────────────────────────────────────────────────────────
-- Um carrossel aprovado tem de sair com "a diagramar":
-- select id, formato, etiquetas from vz_conteudos where formato ilike '%carrossel%';
